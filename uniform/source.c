...
int main(void)
{
	...
	double vxzero[nmax]={}, vyzero[nmax]={};
	...
	printf("density [kg/m^3]:%lf\n", rho);
	printf("characteristic length [m]:%lf\n", length);
	printf("characteristic velocity [m/s]:%lf\n", velocity);
	printf("viscosity [Pa*s]:%lf\n", mu);
	...
	/* boundary condition for U (velocity of x-direction) */
	for(i=1;i<=(int)nbc1;i++)
	{
		fscanf(fp5, "%lf %lf", &nnbc1[i], &vbc1[i]);
	}
	/* boundary condition for V (velocity of y-direction) */
	for(i=1;i<=(int)nbc2;i++)
	{
		fscanf(fp6, "%lf %lf", &nnbc2[i], &vbc2[i]);
	}
	/* boundary condition for P (pressure) */
	for(i=1;i<=(int)nbc3;i++)
	{
		fscanf(fp7, "%lf %lf", &nnbc3[i], &vbc3[i]);
	}
	/* boundary condition for natural boundary condition */
	...
	while(time<=tmax){
		...
		if(time<tmax && res > std)
		{
			...
			/* 8. output file with format of ParaView */
			d = div(stepnum, 5000);
